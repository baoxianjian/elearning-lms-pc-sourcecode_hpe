...
?>
<input type="hidden" id="indexUrl" value="<?=Yii::$app->urlManager->createUrl(['tree-node-content/company-dictionary']);?>"/>
...
//            $.pjax.reload({container:"#gridframe"});
        var ajaxUrl = "<?=Url::toRoute(['company-dictionary/list','TreeNodeKid'=>$selectNodeId])?>";
//        alert('ajaxUrl:'+ajaxUrl);
    ...
    function exportForm()
    {
        var ajaxUrl = "<?=Url::toRoute(['company-dictionary/export','TreeNodeKid'=>$selectNodeId])?>";
...
//                $('#'+modalId).modal('hide');
        ...
        {
//                modalClear(modalId);
            modalLoad(modalId, '<?=Yii::$app->urlManager->createUrl(['dictionary/create', 'TreeNodeKid' => $selectNodeId, 'type' => 'company'])?>');
        }
...
<?php echo $this->render('_search', ['model' => $searchModel, 'includeSubNode' => $includeSubNode, 'dictionaryCategoryModel' => $dictionaryCategoryModel]); ?>
<!-- /.panel-heading -->
<div class="company-dictionary-body">
    <?
    $gridColumns = [
        [
            'name' => 'selectedIds',
...
//                                'data-target'=>'#updateModal',
                                'onclick'=>'loadModalFormData("updateModal","'. Yii::$app->urlManager->createUrl(['dictionary/update','id'=>$key, 'type' => 'company']).'");'
                            ]);
                },
...
//                    'data-target'=>'#addModal',
            'onclick'=>'loadModalFormData("addModal","'. Yii::$app->urlManager->createUrl(['dictionary/create', 'TreeNodeKid' => $selectNodeId,'type'=>'company']) .'");'
    ...
    echo TGridView::widget([
        ...
        'toolbar' => [
            ['content'=>
                ...
                $pageButton
                .' '.
                Html::button('<i class="glyphicon glyphicon-export"></i> '.Yii::t('common', 'export_button'),[
                    'title'=>Yii::t('common', 'export_button'), 'class'=>'btn btn-default blueBtn',
                    'onclick'=>'exportForm();'
                ])
            ],
//            '{export}',
