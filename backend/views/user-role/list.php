...
?>
<input type="hidden" id="indexUrl" value="<?=Yii::$app->urlManager->createUrl(['tree-node-content/user-role']);?>"/>
...
<script>
    ...
    //            $.pjax.reload({container:"#gridframe"});
        var ajaxUrl = "<?=Url::toRoute(['user-role/list','TreeNodeKid'=>$selectNodeId])?>";
    ...
    {
        //alert($("#clientform-user-domain").length);
        if ($("#clientform-user-domain").length > 0)
        {
    ...
    function ContinueSubmit(frameId,modalId,isClose,isErrorSubmit,kid,tabNumber)
    {
        //alert('ContinueSubmit');
        //alert('kid:'+kid);
        //alert('tabNumber:'+tabNumber);
        if (tabNumber == "0") {
            //alert($("#clientform-user-orgnization").length);
            if ($("#clientform-user-domain").length > 0) {
                submitModalFormWithContinue(frameId, "clientform-user-company", modalId, isClose, isErrorSubmit, '1');
            }
            else {
                submitModalForm(frameId, "clientform-user-company", modalId, isClose, isErrorSubmit, kid, '1');
            }
        }
        else if (tabNumber == "1") {
            //alert($("#clientform-user-orgnization").length);
            if ($("#clientform-user-orgnization").length > 0) {
                submitModalFormWithContinue(frameId, "clientform-user-domain", modalId, isClose, isErrorSubmit, '2');
            }
            else {
                submitModalForm(frameId, "clientform-user-domain", modalId, isClose, isErrorSubmit, kid, '2');
            }
        }
        else {
            submitModalForm(frameId, "clientform-user-orgnization", modalId, isClose, isErrorSubmit, kid, '3');
        }
    ...
    //                $('#'+modalId).modal('hide');
        ...
        {
    //                modalClear(modalId);
            modalLoad(modalId,'<?=Yii::$app->urlManager->createUrl(['user-role/create','TreeNodeKid'=>$selectNodeId])?>');
        }
...
//                                'data-target'=>'#viewModal',
                                'onclick'=>'loadModalFormData("viewModal","'. Yii::$app->urlManager->createUrl(['user-role/view','userId'=>$key]).'");'
                            ]);
                },
...
//                                'data-toggle'=>'modal',
//                                'data-target'=>'#viewModal',
                                'onclick'=>'loadModalFormData("updateModal","'. Yii::$app->urlManager->createUrl(['user-role/update','userId'=>$key]).'");'
                            ]);
                },
...
//                'headerOptions' => ['width' => '80'],
    ...
    <?
    $contentName = Yii::t('backend', 'user_role');
