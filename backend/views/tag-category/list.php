...
?>
<input type="hidden" id="indexUrl" value="<?=Yii::$app->urlManager->createUrl(['tag-category/index']);?>"/>
<script>
...
//            $.pjax.reload({container:"#gridframe"});
        ...
        var ajaxUrl = "<?=Url::toRoute(['tag-category/list'])?>";
...
//        alert('ajaxUrl:'+ajaxUrl);
    ...
    function exportForm()
    {
        var ajaxUrl = "<?=Url::toRoute(['tag-category/export'])?>";
...
//                $('#'+modalId).modal('hide');
        ...
        {
//                modalClear(modalId);
            modalLoad(modalId,'<?=Yii::$app->urlManager->createUrl(['tag-category/create'])?>');
        }
...
//                                'data-target'=>'#viewModal',
                                'onclick'=>'loadModalFormData("viewModal","'. Yii::$app->urlManager->createUrl(['tag-category/view','id'=>$key]).'");'
                            ]);
                },
                ...
                //                                'data-target'=>'#updateModal',
                                    'onclick' => 'loadModalFormData("updateModal","' . Yii::$app->urlManager->createUrl(['tag-category/update', 'id' => $key]) . '");'
                                ]);
                    }
                },
                        ...
                        return
                            Html::a('<span class="glyphicon glyphicon-trash"></span>', '#',
                                ['id' => 'DeleteButton', 'title' => Yii::t('common', 'delete_button'),
                                    'onclick' => 'deleteButton("' . $key . '","' . Yii::$app->urlManager->createUrl(['tag-category/delete', 'id' => $key]) . '");'
                                ]);
                    }
                },
            ],
//                'headerOptions' => ['width' => '80'],
    ...
    <?php
    $contentName = Yii::t('common', 'tag_category');
...
//                    'data-target'=>'#addModal',
                    'onclick'=>'loadModalFormData("addModal","'. Yii::$app->urlManager->createUrl(['tag-category/create']) .'");'
//                    'onclick'=>'updateFormData("'. Yii::$app->urlManager->createUrl(['tag-category/update','id'=>43]).'");'
                ])
                .' '.
                Html::button('<i class="glyphicon glyphicon-minus"></i> '.Yii::t('common', 'batch_delete_button'),[
                    ...
                    'onclick'=>'batchDeleteButton("'. Yii::$app->urlManager->createUrl('tag-category/batch-delete').'");'
                ])
                .' '.
                $pageButton
                .' '.
