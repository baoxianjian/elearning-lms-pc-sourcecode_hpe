...
?>
<input type="hidden" id="indexUrl" value="<?=Yii::$app->urlManager->createUrl(['tree-node-content/position']);?>"/>
...
//            $.pjax.reload({container:"#gridframe"});
        var ajaxUrl = "<?=Url::toRoute(['position/list','TreeNodeKid'=>$selectNodeId])?>";
//        alert('ajaxUrl:'+ajaxUrl);
    ...
    function exportForm()
    {
        var ajaxUrl = "<?=Url::toRoute(['position/export','TreeNodeKid'=>$selectNodeId])?>";
...
//                $('#'+modalId).modal('hide');
        ...
        {
//                modalClear(modalId);
            modalLoad(modalId,'<?=Yii::$app->urlManager->createUrl(['position/create','TreeNodeKid'=>$selectNodeId])?>');
        }
...
//                                'data-target'=>'#viewModal',
                                'onclick'=>'loadModalFormData("viewModal","'. Yii::$app->urlManager->createUrl(['position/view','id'=>$key]).'");'
                            ]);
                },
...
//                                'data-target'=>'#updateModal',
                                    'onclick' => 'loadModalFormData("updateModal","' . Yii::$app->urlManager->createUrl(['position/update', 'id' => $key]) . '");'
                                ]);
                    }
                },
                ...
                //                                        'data-target'=>'#moveModal',
                                        'onclick' => 'loadModalFormData("moveModal","' . Yii::$app->urlManager->createUrl(['position/move', 'id' => $key]) . '");'
                                    ]);
                        }
                    }
                },
                        ...
                        return
                            Html::a('<span class="' . $class . '"></span>', '#',
                                ['id' => 'StatusButton', 'title' => $title,
                                    'onclick' => 'statusButton("' . $status . '","' . Yii::$app->urlManager->createUrl(['position/status', 'id' => $key, 'status' => $status]) . '");'
                                ]);
                    }
                },
                        ...
                        return
                            Html::a('<span class="glyphicon glyphicon-trash"></span>', '#',
                                ['id' => 'DeleteButton', 'title' => Yii::t('common', 'delete_button'),
                                    'onclick' => 'deleteButton("' . $key . '","' . Yii::$app->urlManager->createUrl(['position/delete', 'id' => $key]) . '");'
                                ]);
                    }
                },
            ],
//                'headerOptions' => ['width' => '80'],
    ...
    <?
    $contentName = Yii::t('backend', 'position');
    ...
    $buttonDropdownItems[] = [
        ...
        'linkOptions'=>
            [
                ...
                'onclick'=>'batchDeleteButton("'. Yii::$app->urlManager->createUrl('position/batch-delete').'");'
            ]
    ];
    ...
    $buttonDropdownItems[] =  [
        ...
        'linkOptions'=>
            [
                ...
                'onclick'=>'batchOperateButton("stop","'. Yii::$app->urlManager->createUrl(['position/status','status' => '2']).'");'
            ]
    ];
    ...
    $buttonDropdownItems[] =  [
        ...
        'linkOptions'=>
            [
                ...
                'onclick'=>'batchOperateButton("start","'. Yii::$app->urlManager->createUrl(['position/status','status' => '1']).'");'
            ]
    ];
        ...
        $buttonDropdownItems[] = [
            ...
            'linkOptions' =>
                [
                    ...
                    'onclick' => 'batchOperateButton("moveModal","' . Yii::$app->urlManager->createUrl('position/move') . '");'
                ]
        ];
//    }
...
//                    'data-target'=>'#addModal',
            'onclick'=>'loadModalFormData("addModal","'. Yii::$app->urlManager->createUrl(['position/create','TreeNodeKid'=>$selectNodeId]) .'");'
        ]);
    }
    else
    {
