...
?>
<input type="hidden" id="indexUrl" value="<?=Yii::$app->urlManager->createUrl(['tree-node-content/user-reporting-manager']);?>"/>
...
//            $.pjax.reload({container:"#gridframe"});
        var ajaxUrl = "<?=Url::toRoute(['user-reporting-manager/list','TreeNodeKid'=>$selectNodeId])?>";
...
//                $('#'+modalId).modal('hide');
        ...
        {
//                modalClear(modalId);
            modalLoad(modalId,'<?=Yii::$app->urlManager->createUrl(['user-reporting-manager/create','TreeNodeKid'=>$selectNodeId])?>');
        }
...
//                                'data-target'=>'#viewModal',
                                'onclick'=>'loadModalFormData("viewModal","'. Yii::$app->urlManager->createUrl(['user-reporting-manager/view','userId'=>$key]).'");'
                            ]);
                },
...
//                                'data-toggle'=>'modal',
//                                'data-target'=>'#viewModal',
                                'onclick'=>'loadModalFormData("updateModal","'. Yii::$app->urlManager->createUrl(['user-reporting-manager/update','userId'=>$key]).'");'
                            ]);
                },
...
//                'headerOptions' => ['width' => '80'],
    ...
    <?
    $contentName = Yii::t('backend', 'user_reporting_manager');
