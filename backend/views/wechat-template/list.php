...
?>
<input type="hidden" id="indexUrl" value="<?=Yii::$app->urlManager->createUrl(['tree-node-content/wechat-template']);?>"/>
...
//            $.pjax.reload({container:"#gridframe"});
        var ajaxUrl = "<?=Url::toRoute(['wechat-template/list','TreeNodeKid'=>$selectNodeId])?>";
//        alert('ajaxUrl:'+ajaxUrl);
    ...
    function exportForm()
    {
        var ajaxUrl = "<?=Url::toRoute(['wechat-template/export','TreeNodeKid'=>$selectNodeId])?>";
...
//                $('#'+modalId).modal('hide');
        ...
        {
//                modalClear(modalId);
            modalLoad(modalId,'<?=Yii::$app->urlManager->createUrl(['wechat-template/create','TreeNodeKid'=>$selectNodeId])?>');
        }
...
//                                'data-target'=>'#viewModal',
                                'onclick'=>'loadModalFormData("viewModal","'. Yii::$app->urlManager->createUrl(['wechat-template/view','id'=>$key]).'");'
                            ]);
                },
...
//                                'data-target'=>'#updateModal',
                                'onclick' => 'loadModalFormData("updateModal","' . Yii::$app->urlManager->createUrl(['wechat-template/update', 'id' => $key]) . '");'
                            ]);
                },
...
//                                    'onclick' => 'loadModalFormData("moveModal","' . Yii::$app->urlManager->createUrl(['wechat-template/move', 'id' => $key]) . '");'
//                                ]);
//                    }
//                },
                    ...
                    return
                        Html::a('<span class="' . $class . '"></span>', '#',
                            ['id' => 'StatusButton', 'title' => $title,
                                'onclick' => 'statusButton("' . $status . '","' . Yii::$app->urlManager->createUrl(['wechat-template/status', 'id' => $key, 'status' => $status]) . '");'
                            ]);
                },
                    ...
                    return
                        Html::a('<span class="glyphicon glyphicon-trash"></span>', '#',
                            ['id' => 'DeleteButton', 'title' => Yii::t('common', 'delete_button'),
                                'onclick' => 'deleteButton("' . $key . '","' . Yii::$app->urlManager->createUrl(['wechat-template/delete', 'id' => $key]) . '");'
                            ]);
                },
            ],
//                'headerOptions' => ['width' => '80'],
    ...
    <?
    $contentName = Yii::t('common', 'wechat_template');
    ...
    $buttonDropdownItems[] = [
        ...
        'linkOptions'=>
            [
                ...
                'onclick'=>'batchDeleteButton("'. Yii::$app->urlManager->createUrl('wechat-template/batch-delete').'");'
            ]
    ];
    ...
    $buttonDropdownItems[] =  [
        ...
        'linkOptions'=>
            [
                ...
                'onclick'=>'batchOperateButton("stop","'. Yii::$app->urlManager->createUrl(['wechat-template/status','status' => '2']).'");'
            ]
    ];
    ...
    $buttonDropdownItems[] =  [
        ...
        'linkOptions'=>
            [
                ...
                'onclick'=>'batchOperateButton("start","'. Yii::$app->urlManager->createUrl(['wechat-template/status','status' => '1']).'");'
            ]
    ];
...
//                    'onclick' => 'batchOperateButton("moveModal","' . Yii::$app->urlManager->createUrl('wechat-template/move') . '");'
//                ]
//        ];
//    }
