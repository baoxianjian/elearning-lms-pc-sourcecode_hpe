...
//
function SCORMapi1_3(courseRegId,courseCompleteProcessId,courseCompleteFinalId, def, cmiobj, cmiint, cmicommentsuser, cmicommentslms, cmistring256, cmistring4096, scormdebugging, scormauto, scormid, recordUrl, sessionKey, scoid, attempt, viewmode, modResId, currentorg, autocommit) {
...
//        'fill-in':{'pre':'^(((\{case_matters=(true|false)\})(\{order_matters=(true|false)\})?)|((\{order_matters=(true|false)\})(\{case_matters=(true|false)\})?))(.*?)$',
    ...
    for(defScoId in def){
        datamodel[defScoId] = {
            ...
            'cmi.completion_status':{'defaultvalue':def[defScoId]['cmi.completion_status'], 'format':CMICStatus, 'mod':'rw'},
            'cmi.completion_threshold':{'defaultvalue':def[defScoId]['cmi.completion_threshold'], 'mod':'r'},
            'cmi.credit':{'defaultvalue':def[defScoId]['cmi.credit'], 'mod':'r'},
            'cmi.entry':{'defaultvalue':def[defScoId]['cmi.entry'], 'mod':'r'},
            'cmi.exit':{'defaultvalue':def[defScoId]['cmi.exit'], 'format':CMIExit, 'mod':'w'},
            ...
            'cmi.launch_data':{'defaultvalue':def[defScoId]['cmi.exit'], 'mod':'r'},
            'cmi.learner_id':{'defaultvalue':def[defScoId]['cmi.learner_id'], 'mod':'r'},
            'cmi.learner_name':{'defaultvalue':def[defScoId]['cmi.learner_name'], 'mod':'r'},
            'cmi.learner_preference._children':{'defaultvalue':student_preference_children, 'mod':'r'},
            'cmi.learner_preference.audio_level':{'defaultvalue':def[defScoId]['cmi.learner_preference.audio_level'], 'format':CMIDecimal, 'range':audio_range, 'mod':'rw'},
            'cmi.learner_preference.language':{'defaultvalue':def[defScoId]['cmi.learner_preference.language'], 'format':CMILang, 'mod':'rw'},
            'cmi.learner_preference.delivery_speed':{'defaultvalue':def[defScoId]['cmi.learner_preference.delivery_speed'], 'format':CMIDecimal, 'range':speed_range, 'mod':'rw'},
            'cmi.learner_preference.audio_captioning':{'defaultvalue':def[defScoId]['cmi.learner_preference.audio_captioning'], 'format':CMISInteger, 'range':text_range, 'mod':'rw'},
            'cmi.location':{'defaultvalue':def[defScoId]['cmi.location'], 'format':CMIString1000, 'mod':'rw'},
            'cmi.max_time_allowed':{'defaultvalue':def[defScoId]['cmi.max_time_allowed'], 'mod':'r'},
            'cmi.mode':{'defaultvalue':def[defScoId]['cmi.mode'], 'mod':'r'},
            ...
            'cmi.progress_measure':{'defaultvalue':def[defScoId]['cmi.progress_measure'], 'format':CMIDecimal, 'range':progress_range, 'mod':'rw'},
            'cmi.scaled_passing_score':{'defaultvalue':def[defScoId]['cmi.scaled_passing_score'], 'format':CMIDecimal, 'range':scaled_range, 'mod':'r'},
            'cmi.score._children':{'defaultvalue':score_children, 'mod':'r'},
            'cmi.score.scaled':{'defaultvalue':def[defScoId]['cmi.score.scaled'], 'format':CMIDecimal, 'range':scaled_range, 'mod':'rw'},
            'cmi.score.raw':{'defaultvalue':def[defScoId]['cmi.score.raw'], 'format':CMIDecimal, 'mod':'rw'},
            'cmi.score.min':{'defaultvalue':def[defScoId]['cmi.score.min'], 'format':CMIDecimal, 'mod':'rw'},
            'cmi.score.max':{'defaultvalue':def[defScoId]['cmi.score.max'], 'format':CMIDecimal, 'mod':'rw'},
            'cmi.session_time':{'format':CMITimespan, 'mod':'w', 'defaultvalue':'PT0H0M0S'},
            'cmi.success_status':{'defaultvalue':def[defScoId]['cmi.success_status'], 'format':CMISStatus, 'mod':'rw'},
            'cmi.suspend_data':{'defaultvalue':def[defScoId]['cmi.suspend_data'], 'format':CMIString64000, 'mod':'rw'},
            'cmi.time_limit_action':{'defaultvalue':def[defScoId]['cmi.time_limit_action'], 'mod':'r'},
            'cmi.total_time':{'defaultvalue':def[defScoId]['cmi.total_time'], 'mod':'r'},
            'adl.nav.request':{'defaultvalue':'_none_', 'format':NAVEvent, 'mod':'rw'}
        };
    }

    var cmi, adl, errorCode = "0";

    var currentCompletionStatus = "";
    var currentSuccessStatus = "";
    ...
    function initdatamodel(scoid){
        ...
        //alert("cmi.completion_status:"+cmi.completion_status);

        currentCompletionStatus = cmi.completion_status;
        currentSuccessStatus = cmi.success_status;
    }
    ...
    function Initialize (param) {
        //alert('Initialize param:'+param);
        //scoid = scorm_current_node ? scorm_current_node.scoid : scoid;
        //alert('Initialize scoid:'+scoid);
        errorCode = "0";
        LogAPICall("Initialize", "scoid", scoid, errorCode, "");
        initdatamodel(scoid);
        ...
        if (param == "") {
            if ((!Initialized) && (!Terminated)) {
                ...
                errorCode = "0";
                if (scormdebugging) {
                    LogAPICall("Initialize", "param", param, errorCode, "");
                }
                return "true";
            } else {
                if (Initialized) {
    ...
    function Terminate (param) {
        ...
        if (param == "") {
            if ((Initialized) && (!Terminated)) {
                ...
                if (scormdebugging) {
                    LogAPICall("Terminate", "AJAXResult", result, errorCode, "");
                }
                if ('true' == result) {
                    Initialized = false;
                    Terminated = true;
        ...
        if (scormdebugging) {
            LogAPICall("Terminate", "param", param, errorCode, "");
        }
        return "false";
    }

    function GetValue (element) {
        //alert('GetValue element:'+element);
        errorCode = "0";
        diagnostic = "";
        if ((Initialized) && (!Terminated)) {
            if (element != "") {
    ...
    function SetValue (element,value) {
        //alert('SetValue element:'+element+",value:"+value);
        errorCode = "0";
        diagnostic = "";
        if ((Initialized) && (!Terminated)) {
            if (element != "") {
                        ...
                        if ((matches != null) && ((matches.join('').length > 0) || (value.length == 0))) {
                            ...
                            if (errorCode == "0") {
                                if (autocommit && !(SCORMapi1_3.timeout)) {
                                    ...
                                    SCORMapi1_3.timeout = setTimeout("API_1484_11.Commit('')", 10000);
    ...
    function CRremovePrefixes (node) {
        //alert('CRremovePrefixes node:'+node);
    ...
    function CRcheckValueNodes(element, interactiontype, nodes, value, errorCode) {
        //alert('CRcheckValueNodes node:'+element);
        var expression = new RegExp(correct_responses[interactiontype].format);
        for (var i = 0; (i < nodes.length) && (errorCode == "0"); i++) {
    ...
    function Commit (param) {
        //alert('Commit param:'+param);
        if (SCORMapi1_3.timeout) {
            //alert('SCORMapi1_3.timeout');
            //SCORMapi1_3.timeout.clear();
        ...
        if (param == "") {
            if ((Initialized) && (!Terminated)) {
                ...
                errorCode = ('true' == result) ? '0' : '101'; // General exception for any AJAX fault
                if (scormdebugging) {
                    LogAPICall("Commit", "AJAXResult", result, errorCode, "");
                }
                if ('false' == result) {
        ...
        if (scormdebugging) {
            LogAPICall("Commit", "param", param, errorCode, "");
        }
        return "false";
    }
    ...
    function GetErrorString (param) {
        //alert('GetErrorString param:'+param);
        if (param != "") {
            var errorString = "";
            switch(param) {
                case "0":
            ...
            if (scormdebugging) {
                if (param != "0") {
                    LogAPICall("GetErrorString", "param", param, param, errorString[param]);
                }
            }
            return errorString;
        }
            ...
            // if (scormdebugging) {
            //     LogAPICall("GetErrorString", param,  "", "0", "No error string found!");
            // }
            return "";
        }
    }
    ...
    function GetDiagnostic(param) {
        //alert('GetDiagnostic param:'+param);
        if (param == "") {
            param = errorCode;
        }
        if (scormdebugging) {
            LogAPICall("GetDiagnostic", "param", param, param, "");
        }
        return param;
    }
    ...
    function StoreData(data,storetotaltime) {
        ...
        if (eval(element) != datamodel[scoid][element].defaultvalue) {
            postData[underscore(element)] = encodeURIComponent(eval(element));
        }

        if (VIEWMODE == "normal" && courseRegId != "" && postDataCount > 0) {
            //var newRecordUrl  = recordUrl + datastring;
            ////alert("newRecordUrl:"+newRecordUrl);
            ////alert(recordUrl);
            ...
            ajaxData(recordUrl,
                ...
                function(data){
                    ...
                    if (currentCompletionStatus !=  cmi.completion_status || currentSuccessStatus !=  cmi.success_status)
                    {
                        currentCompletionStatus = cmi.completion_status;
                        currentSuccessStatus = cmi.success_status;
...
function scorm_api_init(courseRegId,courseCompleteProcessId,courseCompleteFinalId,def, cmiobj, cmiint, cmicommentsuser, cmicommentslms, cmistring256, cmistring4096, scormdebugging, scormauto, scormid, recordUrl, sessionKey, scoid, attempt, viewmode, modResId, currentorg, autocommit) {
    ...
    window.API_1484_11 = new SCORMapi1_3(courseRegId,courseCompleteProcessId,courseCompleteFinalId, def, cmiobj, cmiint, cmicommentsuser, cmicommentslms, cmistring256, cmistring4096, scormdebugging, scormauto, scormid, recordUrl, sessionKey, scoid, attempt, viewmode, modResId, currentorg, autocommit);
}
