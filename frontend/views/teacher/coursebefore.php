		...
		<div role="tab" id="headingOne">
			<ul class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingOne" id="collapseExample">
				 ...
				 if (!empty($course)) :
					 foreach($course as $t):
				 ?>
				<li class="pathStep"><span class="step ">【<?=Yii::t('common', 'face_to_face')?>】<?=$t->course_name ?><?if($t->open_start_time): ?>【<?=date('Y-m-d', $t->open_start_time) ?>】<?endif;?></span>
				<span class="stepTime pull-right">
					<a href="<?=Yii::$app->urlManager->createUrl(['teacher/edit','id'=>$t->kid])?>" style="margin-right: 10px;"><?=Yii::t('common', 'edit_button')?></a>
...
<script>
    $(function(){
        $("#courseAward .pagination").on('click', 'a', function(e){
            e.preventDefault();
            ajaxGet($(this).attr('href'), "courseAward");
        });
        $("#courseAward .preview").on('click', function(e){
            e.preventDefault();
