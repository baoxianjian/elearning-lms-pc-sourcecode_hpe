...
if($is_course=="yes"){
	...
	if($course_type=='0'){
		...
		$this->params['breadcrumbs'][] = ['label'=>Yii::t('frontend', 'inv_result_nimingtoupiaojieguo'),];
...
<div class="container">
      ...
      <div class="col-md-12 col-sm-12">
        <div class="courseInfo">
          <ul class="nav nav-tabs hotNews" role="tablist" id="myTab">
            ...
            <li role="presentation"><a href="#survey_stat_detail" aria-controls="survey_stat_detail" role="tab" data-toggle="tab"><?=Yii::t('frontend', 'inv_result_nimingtoupiaoxiangqing')?></a></li>
          </ul>
                ...
                <div class="panel-body">
                  <div class="col-md-8 col-md-offset-2">
                    ...
                    <p><?=Yii::t('frontend', 'inv_result_yiyou_special_1_start')?><strong class="strong"><?=$results['sub_result_arr_num'] ?></strong><?=Yii::t('frontend', 'inv_result_rencanyu_special_1_end')?>
                     <? if($is_course!="yes"){ ?> 
                    <?=Yii::t('frontend', 'inv_result_youxiaoqi')?>：<?=$results['start_at'] ?> - <?=$results['end_at'] ?>
                     <?} ?></p>
                    <hr>
   ...
   <? if($is_course!="yes"){ ?>  
  var list_url = "<?=Url::toRoute(['investigation-result/v-nm-list'])?>"+"?iid=<?=$iid ?>"+"&rel_users_type=yes&all_user_count_is_0=<?=$results['all_user_count_is_0'] ?>";
  <?} else{?>
  var list_url = "<?=Url::toRoute(['investigation-result/v-nm-list'])?>"+"?iid=<?=$iid ?>&&course_id=<?=$id ?>"+"&rel_users_type=yes&all_user_count_is_0=<?=$results['all_user_count_is_0'] ?>";
  <?}?>		
  ...
  $(document).ready(function () {
      ...
      $("#rel_users_id").change(function(){
    	  ...
    	  <? if($is_course!="yes"){ ?>  
    	  list_url = "<?=Url::toRoute(['investigation-result/v-nm-list'])?>"+"?iid=<?=$iid ?>"+"&rel_users_type="+rel_users_id_val+"&all_user_count_is_0=<?=$results['all_user_count_is_0'] ?>";
    	  <?} else{?>
    	  list_url = "<?=Url::toRoute(['investigation-result/v-nm-list'])?>"+"?iid=<?=$iid ?>&&course_id=<?=$id ?>"+"&rel_users_type="+rel_users_id_val+"&all_user_count_is_0=<?=$results['all_user_count_is_0'] ?>";
    	  <?}?>		
