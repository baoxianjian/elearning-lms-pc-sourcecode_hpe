...
if($is_course=="yes"){
	if($course_type=='0'){
		...
		$this->params['breadcrumbs'][] = ['label'=>Yii::t('frontend', 'inv_result_shimingwenjuanjieguo'),];
      ...
      <div class="col-md-12 col-sm-12">
        <div class="courseInfo">
          <ul class="nav nav-tabs hotNews" role="tablist" id="myTab">
            ...
            <li role="presentation"><a href="#survey_stat_detail" aria-controls="survey_stat_detail" role="tab" data-toggle="tab"><?=Yii::t('frontend', 'inv_result_shimingwenjuanxiangqing')?></a></li>
          </ul>
                  ...
                  <div class="actionBar">
                    ...
                    <form class="form-inline pull-right">
                      <div class="form-group">
                        ...
                        <button type="reset" id="reset_id" class="btn btn-default pull-right"><?=Yii::t('frontend', 'reset')?></button>
                        <button type="button" id="query_id" class="btn btn-primary pull-right" style="margin-left:10px;"><?=Yii::t('common', 'search')?></button>
                      </div>
                    </form>
                  </div>
                  <div id="list"></div>
  ...
  $('.moreDetailsBtn').bind('click', function() {
    ...
    if (thisBtn.text() == "<?=Yii::t('frontend', 'inv_result_zhankai')?>") {
  	 ...
  	 function FmodalLoadData(target, url)
	 {
  	 ...
  	 <? if($is_course!="yes"){ ?>  
  	var list_url = "<?=Url::toRoute(['investigation-result/s-sm-list'])?>"+"?iid=<?=$iid ?>"+"&rel_users_type=yes&all_user_count_is_0=<?=$results['all_user_count_is_0'] ?>";
  	 <?} else{?>
 	var list_url = "<?=Url::toRoute(['investigation-result/s-sm-list'])?>"+"?iid=<?=$iid ?>&&course_id=<?=$id ?>"+"&rel_users_type=yes&all_user_count_is_0=<?=$results['all_user_count_is_0'] ?>";
  	 <?}?>		
    ...
    $(document).ready(function () {
        ...
        $("#query_id").click(function(){
        	  ...
        	  var user_info_id=$("#user_info_id").val();
         	  ...
         	  <? if($is_course!="yes"){ ?>  
         	  list_url = "<?=Url::toRoute(['investigation-result/s-sm-list'])?>"+"?iid=<?=$iid ?>"+"&rel_users_type="+rel_users_id_val+"&all_user_count_is_0=<?=$results['all_user_count_is_0'] ?>";
         	  <?} else{?>
         	  list_url = "<?=Url::toRoute(['investigation-result/s-sm-list'])?>"+"?iid=<?=$iid ?>&&course_id=<?=$id ?>"+"&rel_users_type="+rel_users_id_val+"&all_user_count_is_0=<?=$results['all_user_count_is_0'] ?>";
         	  <?}?>		
