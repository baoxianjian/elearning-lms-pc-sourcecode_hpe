...
<div class="panel-default scoreList " >
                      ...
                      <div class="form-group">
                       <input type="text" id="course_name_id" class="form-control clickSearch" placeholder="<?= Yii::t('frontend','rep_input_course_name_or_code') ?>">
                               ...
                               <button type="button" class="btn btn-primary pull-right" style="margin-left:10px;" id="cvs_export"><?=Yii::t('common', 'export_button')?></button>
                     		    <button type="button" class="btn btn-primary pull-right" style="margin-left:10px;" id="query_face"><?=Yii::t('frontend', 'tag_query')?></button>
                       </div>
                    </form>   
                  ...
                  <table class="table table-bordered table-hover table-striped sortable table-center" id="table_body">
                     <thead>
                       <tr id="table_header">
	                      ...
	                      <th data-defaultsort="asc"><?= Yii::t('frontend', 'rep_enroll_num') ?></th>
	                      <th><?= Yii::t('frontend', 'rep_finish_number') ?></th>
	                      <th><?= Yii::t('frontend', 'rep_not_qualify_rate') ?></th>
	                      <th><?= Yii::t('frontend', 'rep_certification_rate') ?></th>
                   		 </tr>
                    </thead>
                     <tbody>
                     </tbody>
  ...
  var queryList;
  function flash(){
		   var data_url_val="<?=Url::toRoute(['report-new/get-courses',])?>"+"?course_type=1";	
...
//
    ...
    flash();
 	...
 	$("#cvs_export").click(function(){
	         ...
	         iframe.src = "<?=Url::toRoute(['report-new/export-face-course-comp',])?>"+"?course_id="+course_id_val+"&type_param="+type_val;
...
//
    ...
    function reflash(course_id_,type_){
      var ajaxOpt = {
         ...
         url: '<?=Url::toRoute(['report-new/get-face-course-comp-data',])?>',
         data: {course_id:course_id_,type_param:type_},
         async: false,
         success: function(msg){
              $("#table_body tbody").empty();
              var lists=msg.faceCourseComp;        
...
<%_.each(data.datas, function(item) {%>
					 <tr>
                      ...
                      <td><%=item.reg_num%></td>
                      <td class="fakeData" data-fake=" (<%=item.com_num_rate%>%)"><%=item.com_num%></td>
  					  <td class="fakeData" data-fake=" (<%=item.not_qualify_rate%>%)"><%=item.not_qualify%></td>
                      <td class="fakeData" data-fake=" (<%=item.certification_rate%>%)"><%=item.certification%></td>
                    </tr>
 <%});%>
    </script>                 
