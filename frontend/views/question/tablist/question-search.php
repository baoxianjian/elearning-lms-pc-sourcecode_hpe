<form class="form-inline pull-left" style="width:40%; margin:20px 0;" id="minesearch">
    <div class="form-group">
        <input type="text" class="form-control" id="searchcontent" placeholder="<?=Yii::t('frontend', 'title_reply_comment')?>" <? use yii\helpers\Html;

        if(!empty($key)){ echo 'value="'.$key.'"';}?>>
...
<div class="filterBtn pull-right" style="width:50%;" id="minelist">
    <a href="#" class="btnFilter <? if($status == 'mine'){echo 'activeBtn';}?>" data-num="0" id="questionmine" ><?=Yii::t('frontend', 'i_propose')?></a>
    <a href="#" class="btnFilter <? if($status == 'care'){echo 'activeBtn';}?>" data-num="1" id="icare" ><?=Yii::t('frontend', 'i_follow')?></a>
    <a href="#" class="btnFilter <? if($status == 'atme'){echo 'activeBtn';}?>" data-num="2" id="atme" ><?=Yii::t('frontend', 'i_at')?></a>
    <a href="#" class="btnFilter <? if($status == 'answer'){echo 'activeBtn';}?>" data-num="3" id="ianswered" ><?=Yii::t('frontend', 'i_reply')?></a>
</div>
<div class="questionLine" id="questionismine">
    ...
    <?php foreach($data as $k => $v):?>
        <div class="questionBlock" id="<?=$v['kid']?>" type="">
            <h3>
                <a href="<?=Yii::$app->urlManager->createUrl(['question/detail','id'=> "$v[kid]"])?>"><?php echo Html::encode(mb_substr($v['title'],0,20,"utf-8"));if(mb_strlen($v['title'],'utf-8')>20) echo "..."?></a>
            </h3>
            <span class = "setitle"><?=Yii::t('frontend', 'release_at')?>:  <?php echo date("Y-m-d H:i",$v['created_at'])?></span>
            <span class = "setitle"><?php if($v['question_type']==1&&!empty($coursefinal)):?><?= Yii::t('frontend', 'from_text') ?>: 【<?= Yii::t('common', 'course') ?>】<?php echo Html::encode($coursefinal["$v[obj_id]"]); endif;?></span>
        <span>
