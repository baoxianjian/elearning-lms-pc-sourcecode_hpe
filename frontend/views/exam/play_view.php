...
if ($examination_range == 'True') {
    $this->params['breadcrumbs'][] = $examination->title;
...
?>
                                        ...
                                        <div class="row questionGroup_quest" data-num="<?=$j?>">
                                            ...
                                            <div class="col-md-12 col-sm-12">
                                                ...
                                                if ($item['examination_question_type'] == LnExaminationQuestion::EXAMINATION_QUESTION_TYPE_RADIO){
                                                    ...
                                                    <div class="options">
                                                        <label style="margin-right:40px;">
                                                            <input type="radio" class="<?=$item['kid']?>" onclick="return false;" <?=isset($item['is_checked']) && $item['values']?'checked':''?>> <?=Yii::t('frontend', 'exam_right')?>
                                                        </label>
                                                    </div>
                                                    <div class="options">
                                                        <label style="margin-right:40px;">
                                                    ...
                                                    <?php
                                                    $allRight = $item['values'] ? array(Yii::t('frontend', 'exam_wrong')) : array(Yii::t('frontend', 'exam_right'));
                                                    $checked = !isset($item['is_checked']) ? null : ($item['values'] ? Yii::t('frontend', 'exam_right') : Yii::t('frontend', 'exam_wrong'));
                                                ...
                                                <div class="options option_answers">
                                                    ...
                                                    <p><i class="glyphicon glyphicon-remove"></i>
                                                        <?=Yii::t('frontend', 'exam_right_is')?> <strong><?=!empty($allRight) ? join('、',array_unique($allRight)) : $allRight?></strong>, <?=Yii::t('frontend', 'exam_right_is')?> <strong><?= $checked ? (is_array($checked) ? join('、', $checked) : $checked) : '--' ?></strong>, <?=Yii::t('frontend', 'exam_answer_wrong')?>.
                                                    </p>
                                                    <?php
                                                    }else {
                                                        $itemRight[] = $item['kid'];
                                                    ...
                                                    <p><i class="glyphicon glyphicon-ok"></i>
                                                        <?=Yii::t('frontend', 'exam_your_a_right')?>.
                                                    </p>
                                                    <?php
                                                    }
        ...
        <div class="col-md-3 col-xs-12 bd">
            <div class="modal-header">
                <h4 class="modal-title"><?=Yii::t('frontend', 'exam_time_passed')?>: <strong data="<?=$model->examination_duration?>"><?=!empty($model->examination_duration) ? TStringHelper::timeSecondToHMS($model->examination_duration) : '--'?></strong>
                <?php
                if ($backBtn != 'False') {
                ?>
                <a href="<?= Url::toRoute(['/exam/view', 'id' => $examination->kid]) ?>" class="pull-right"><?= Yii::t('common', 'back_button') ?></a>
                <?php
                }
                ?>
                </h4>
