...
$this->params['breadcrumbs'][] = ['label'=>Yii::t('frontend', 'exam_pq_bianji'),];
...
<?= html::jsFile('/static/frontend/js/uuid.js') ?>
     ...
     <input name="kid" type="hidden" id="kid" value="<?=$examPaper['kid'] ?>">
...
-->
    ...
    <div class="row">
              ...
              <div class="form-group">
                ...
                <div class="form-group">
                  <select id="choice_type" class="form-control" name="">
                    <option value=""><?=Yii::t('frontend', 'exam_pq_xuanzetiku')?></option>
      ...
      <div class="col-sm-12 bd" style="margin-bottom:50px;">
        ...
        <div class="centerBtnArea">
          ...
          <a href="###" class="btn btn-sm centerBtn" style="width:10%" id="preview_id"><?= Yii::t('common', 'preview_button') ?></a>
        </div>
 ...
 <!-- 提交 -->
  <div class="ui modal" id="edit_exam_paper_submit_ui" >
 </div>
   ...
   <div id="random_choice_show" class="ui modal">
    ...
    function exam_question_exec_fun(){
        ...
        if(!exam_question_input_val){
       	 ...
       	 app.showMsg("<?=Yii::t('frontend', '{value}_not_null',['value'=>Yii::t('common', 'number')])?>");
        }else{
    ...
    function initRightContent(){
        ...
        $.get("<?=Yii::$app->urlManager->createUrl(['exam-paper-manage/get-edit-exam-paper-questions'])?>"+"?examination_paper_id=<?=$examPaper['kid'] ?>",
    	    	  function(msg){
        	...
        	// var new_array=_.sortBy(msg.result, 'sequence_number'); 
        	 var new_array=_.sortBy(msg.result, function(num){      
            	 return  parseInt(num.sequence_number) ;
             });
        	 ...
        	 for(var i=0;i<new_array.length;i++){
        		var obj=new_array[i];
        		if(obj.relation_type=='0'){
        			 var t2_temp=_.template($("#t2").html(),{variable: 'data'})({datas:[obj]});      			 	
        	         $("#right_questions_ul").append(t2_temp);
        	         result.push({kid:obj.kid});
        	         total_question_num=total_question_num+1;
        	         total_question_score=parseFloat(total_question_score)+parseFloat(obj.default_score);	
        	        	
                }else if(obj.relation_type=='1'){
    	...
    	queryList2 = app.queryList("#right_search_tag");
         ...
         $("#new_exam_paper_submit").click(function(){
        	...
        	//console.log(params);
        	 ...
        	 var url="<?=Yii::$app->urlManager->createUrl(['exam-paper-manage/edit-exam-paper-submit'])?>";
        	 FmodalLoadData1("edit_exam_paper_submit_ui",url,params);
         ...
         $("#removeRightQuestion").click(function(){
           	     ...
           	     if($("input[name='right_checkboxs']:checked").length==0){
        			 app.showMsg("<?=Yii::t('frontend', 'exam_pq_note_16')?>");
        			 return ;	
        		 }
 ...
 <%_.each(data.datas, function(item) {%>
       <div class="centerBtnArea noData"  id="question_no_datas">
                <i class="glyphicon glyphicon-calendar"></i>
                <p><?=Yii::t('frontend', 'exam_pq_note_21')?></p>
        </div> 
 <%});%> 
 </script> 
                    ...
                    <div class="choiceType1">
                      <div class="infoBlock">
                        <div class="row">
                          <div class="col-md-12 col-sm-12">
                            <div class="form-group form-group-sm">
                              <p><?=Yii::t('frontend', 'exam_pq_suijirule_tixing')?> ： <?=Yii::t('frontend', 'exam_pq_yaoqiufenshu')?> ：<%=item.total%><?=Yii::t('frontend', 'exam_fenshu')?> </p>
 							  <p> <?=Yii::t('frontend', 'exam_pq_yiwanchengfenshu')?> ：<%=item.num1%> <?=Yii::t('frontend', 'exam_fenshu')?>    <?=Yii::t('frontend', 'exam_pq_queshao')?><%=item.num2%><?=Yii::t('frontend', 'exam_fenshu')?> </p>
