	    		 ...
	    		 for(var i=0;i<survey_submit_get_obj.question.length;i++){
	    			 ...
	    			 if(survey_submit_to_result_obj.question_type=='2'){
						 ...
						 if($.trim($("#"+survey_submit__question.id+"wendati").val())==""){
							 app.showMsg((i+1)+'.'+survey_submit__question.question_title+"<?=Yii::t('frontend', '{value}_not_fill',['value'=>''])?>");
							 submit_flag=true;
							 break ;
						 }
	    			 ...
	    			 if(survey_submit_to_result_obj.question_type=='0'){
	    				 ...
	    				 if($.trim($("input[name='"+survey_submit__question.id+"danxuanti']:checked").val())==""){
							 app.showMsg((i+1)+'.'+survey_submit__question.question_title+"<?=Yii::t('frontend', '{value}_not_choose',['value'=>''])?>");
							 submit_flag=true;
							 break ;
						 }
...
<div class="row">
    <div class="col-md-12 col-sm-12 centerBtnArea">
     <a onClick="survey_play_submit()" id="survey_play_submit_a" class="btn btn-info btn-sm centerBtn" style="width:30%" data-dismiss="modal" aria-label="Close"><?=Yii::t('common', 'submit')?></a>
    </div>
</div>
 <%});%>
 </script>          