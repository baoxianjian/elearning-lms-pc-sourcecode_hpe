                ...
                <div class="panel-body courseInfoInput">
                    ...
                    <div class="guideBlock resourceBlock">
                        <div class="form-group" style="width:150px;">
                            <select id="courseservice-mod_type" class="form-control" name="LnCourse[mod_type]" style="height: auto;">
                                <option value="0" <?=$model->mod_type==LnCourse::MOD_TYPE_RANDOM?'selected':''?>><?= Yii::t('frontend', 'free_learning') ?></option>
                                <option value="1" <?=$model->mod_type==LnCourse::MOD_TYPE_ORDER?'selected':''?>><?= Yii::t('frontend', 'sequential_learning') ?></option>
                            </select>
                        </div>
                        <input type="hidden" name="is_copy" value="<?=$is_copy?>" />
                        <input type="hidden" name="LnCourse[company_id]" value="<?=$model->company_id?>" />
                        <input type="hidden" name="LnCourse[origin_course_id]" value="<?=$model->origin_course_id?>" />
                        ...
                        <input type="hidden" name="tree_node_id" value="<?=$tree_node_id?>" />
                        ...
                        <?php
                        if (!empty($teacher)){
                            foreach ($teacher as $sval){
                                ?>
                                <input type="hidden" name="teacher_id[]" value="<?=$sval?>" />
                                <?php
                            }
                        }
                        ?>
                                ...
                                foreach($modules as $module){
                                    ...
                                    <li data-id="<?=$i+1?>" class="courseModule">
                                        <div style="display: none">
                                            <input type="hidden" name="resource[<?=$module['mod_num']?>][kid]" class="kid" value="<?=$is_copy == LnCourse::IS_COPY_YES ? null : $module['kid']?>">
                                        ...
                                        <table class="table">
                                            ...
                                            <tr>
                                                ...
                                                <td class="desc">
                                                    <textarea class="mod_desc" id="first_desc" rows="3" name="resource[<?=$module['mod_num']?>][mod_desc]"><?=$module['mod_desc']?></textarea>
                                                </td>
                                            </tr>
                                            <tr>
                        ...
                        <div class="centerBtnArea col-md-12 col-sm-12">
                            ...
                            <a href="###" class="btn" style="position:absolute;right:80px;top:5px"  onclick="loadModalFormData('addModal','<?=Yii::$app->urlManager->createUrl(['resource/component/config-list'])?>',this,'other','other','1');"><?= Yii::t('frontend', 'complete_config') ?></a>
                    ...
                    <div class="pull-right">
                        ...
                        if ($model->status != \common\models\learning\LnCourse::STATUS_FLAG_NORMAL){
                            ...
                            <?= Html::button(Yii::t('frontend', 'cache_course'), [
                                ...
                                'data-src' => Url::toRoute(['/resource/course/manage']),
                                'onclick' => 'saveCourse(0,0)',
                            ]) ?>
                            <?php
                        }
                        if ($is_copy == LnCourse::IS_COPY_NO) {
                            echo Html::button(Yii::t('frontend', 'publish_course'), [
...
<script>
    ...
    $(function() {
        ...
        $("#modList").on("click", ".del", function() {
            ...
            $("input[id^=dir_"+mod_num+"_"+object_id+"]").remove();
        ...
        $("#backEdit").on('click',function(){
            ...
            if ($is_copy == LnCourse::IS_COPY_NO){
            ?>
            $("#courseForm").attr('action', '<?=Url::toRoute([$this->context->id.'/edit'])?>?id='+id).submit();
            <?php
            }else{
            ?>
            $("#courseForm").attr('action', '<?=Url::toRoute([$this->context->id.'/online-copy', 'origin_course_id' => $origin_course_id, 'companyId' => $model->company_id])?>&id='+id).submit();
            <?php
            }
            ?>
        });
    ...
    function saveCourse(status, preview){
        ...
        var url = null;
        if (preview == 1){
            url = '<?=Yii::$app->urlManager->createUrl(['/resource/course/preview'])?>?id='+id+'&isCopy=<?=$is_copy?>';
            ...
            ?>
            url = '<?=Url::toRoute(['/resource/course/copy-save', 'origin_course_id' => $model->kid, 'companyId' => $model->company_id])?>&id='+id;
            <?php
            }else{
            ?>
        ...
        if($('#modList').find('input.mod_num').length == 0){
            app.showMsg("<?= Yii::t('frontend', 'add_{value}',['value'=>Yii::t('frontend','mod')]) ?>！");
            app.hideLoadingMsg();
            return;
        }
        var temp = [];
        ...
        $(".courseModule").each(function(){
            ...
            if (app.stringLength(mod_name) == 0){
                error ++;
            ...
            if (mod_desc != xss_mod_desc){
                ...
                app.showMsg('<?= Yii::t('frontend', '{value}_lillegal_char',['value'=>Yii::t('frontend','mod_title')]) ?>');
                return false;
            }
            var courseware_length = $(this).find('.componentid').length;
            if (courseware_length == 0){
                error ++;
                app.showMsg('<?=Yii::t('frontend', 'mod_no_componet')?>');
                //return false;
            }
            ...
            if (component != componentid){
                error ++;
                app.showMsg('<?=Yii::t('frontend', 'course_resource_complete')?>');
                //return false;
            }
        });
                ...
                if (preview == 0) {
                    ...
                    if (response.result == 'success') {
                        if (status == 1) {
                            ...
                            setTimeout(function () {
                                location.href = '<?=Yii::$app->urlManager->createUrl(['resource/course/manage'])?>';
                            }, 1000);
                        } else {
                            ...
                            <?php
                            //update by baoxianjian 16:00 2016/6/1 暂存后显示预览按钮,不跳转
                            if ($is_copy == LnCourse::IS_COPY_YES && false){
                            ?>
                            setTimeout(function () {
                                location.href = '<?=Yii::$app->urlManager->createUrl(['resource/course/manage'])?>';
                            }, 1000);
    ...
    function loadModalFormData(modalId, url, obj, type, code, window) {
        ...
        if (url.indexOf('?') > 0){
            url += '&isCourseType=<?=LnCourse::COURSE_TYPE_ONLINE?>';
        }else{
            url += '?isCourseType=<?=LnCourse::COURSE_TYPE_ONLINE?>';
        }
        url += '&is_copy=<?=$is_copy?>&companyId=<?=$model->company_id?>';
