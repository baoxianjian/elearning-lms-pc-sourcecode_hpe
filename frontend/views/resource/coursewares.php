...
<script>
    ...
    {
        var ajaxUrl = "<?=Url::toRoute(['resource/coursewares','component_id'=>$component_id,'sequence_number'=>$sequence_number,'domain_id'=>$domain_id])?>";
        var pagesize = $('#pageSizeSelect_grid').val();
        if (typeof pagesize == 'undefined') pagesize = 10;
        ajaxUrl = urlreplace(ajaxUrl,'PageSize', pagesize);
        ajaxUrl = urlreplace(ajaxUrl,'CoursewareService[courseware_name]',$('#searchText').val());
        ajaxGet(ajaxUrl, 'componentList');
    }
    function resetForm1(){
...
<div class="form-inline pull-right searchForm">
    ...
    <?= $form->field($searchModel, 'courseware_name')->textInput(['id'=>'searchText','placeholder'=>Yii::t('frontend','code_name_vendor')])->label(false) ?>
    ...
    $gridColumns = [
        [
            ...
            'value' => function($model,$key){
                return '<a href="javascript:;">'.$model->getCoursewareIcon().'&nbsp;'.$model->courseware_name.'</a><div class="addAction pull-right"><i class="glyphicon glyphicon-plus"></i></div><input type="hidden" class="componentid" name="coursewares[]" value="'.$model->kid.'"/>';
            },
