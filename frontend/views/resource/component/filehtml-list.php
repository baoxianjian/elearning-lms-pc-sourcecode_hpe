...
<script>
    ...
    {
        var ajaxUrl = "<?=Url::toRoute(['resource/component/file-html','component_id'=>$component_id,'sequence_number'=>$sequence_number,'domain_id'=>$domain_id,'courseware_type'=>$courseware_type,'entry_mode'=>$entry_mode])?>";
...
<div class="form-inline pull-right searchForm">
    ...
    <?= $form->field($searchModel, 'courseware_name')->textInput(['id'=>'searchText2','placeholder'=>Yii::t('frontend', 'what_want_to_search')])->label(false) ?>
    ...
    $gridColumns = [
        [
            ...
            'value' => function($model,$key){
                return '<a href="javascript:;"> <i class="icon iconfont">မ</i>&nbsp;'.$model->courseware_name.'</a><div class="addAction pull-right"><i class="glyphicon glyphicon-plus" ></i></div><input type="hidden" class="componentid" data-modnum="<?=$params[mod_num]?>" data-restitle="'.$model->courseware_name.'" data-compnenttitle="<?=$component->title?>"  data-completerule="<?=$component->complete_rule?>" data-isscore="<?=$component->is_record_score?>" name="coursewares[]" value="'.$model->kid.'"/>';
            },
