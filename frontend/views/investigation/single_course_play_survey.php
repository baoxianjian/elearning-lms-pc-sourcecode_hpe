...
<!-- 预览问卷的弹出窗口 -->
  ...
  <div class="courseInfo">
         ...
         function get_play_survey_result_show(){
        	 ...
        	 $.get("<?=Url::toRoute(['investigation/get-single-sub-survey-result',])?>"+"?id=<?=$id?>&investigation_id=<?=$id?>",function(msg){
		         ...
		         if(survey_submit_count==0){
		        	 survey_submit_count++;
		        	  $.ajax({
			  			   ...
			  			   data:{param:survey_submit_to_result_obj_arr,"user_id":"<?=$user_id?>",'investigation_type':'survey'},
