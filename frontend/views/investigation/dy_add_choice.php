...
?>
                    ...
                    <a  class="btn btn-sm pull-right editPanel_cancelBtn" id="deleteDyAddSurveyChoiceMain"><?= Yii::t('common', 'delete_button') ?></a>
                    <form id="add_dy_survey_form_id">
                    <div class="row">
                     ...
                     <input type="hidden" value="<?=$result['id'] ?>" id="dy_add_choice_id"/>
                     <div id="dy_add_choice_content_div">
                      ...
                      <div class="col-md-12 col-sm-12">
                        <div class="form-group form-group-sm">
                          ...
                          <div class="col-sm-9">
                            ...
                            <a  onclick="delete_dy_add_choice_option(this)" class="btn btn-default btn-sm glyphicon glyphicon-remove deleteBtn" title="<?= Yii::t('common', 'delete_button') ?>"></a>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="row">
                      <div class="col-md-12 col-sm-12 centerBtnArea">
                        <a  id="addDySurveyChoiceMain" class="btn btn-default btn-sm centerBtn" style="width:30%"><?=Yii::t('frontend', 'be_sure')?></a>
                         <a  id="addDySurveyChoiceItem" class="btn btn-default btn-sm centerBtn" style="width:30%"><?=Yii::t('frontend', 'add_option')?></a>
                      </div>
                    </div>
</form>

<script id="dy_add_t5" type="text/template">
...
<script id="dy_add_t6" type="text/template">
 <%_.each(data.datas, function(item) {%>
...
<script id="dy_add_t8" type="text/template">
 <%_.each(data.datas, function(item) {%>
 ...
 <%});%>
  ...
  function delete_dy_add_choice_option(node){
	  $(node).parent().parent().parent().parent().remove();
  }
  $(function(){


	 // window.validation_edit_survey_dy_add_choice =app.creatFormValidation($("#add_dy_survey_form_id"));  

    $("#deleteDyAddSurveyChoiceMain").bind('click', function() {
    ...
    $("#addDySurveyChoiceMain").click(function(){
	     ...
	     var question_type_val=$("#add_dy_survey_form_id input[name='question_type']:checked").val();
	     var question_title_val=app.clean($("#add_dy_survey_form_id input[name='question_title']").val());
	     var option_title_arrs=$("#add_dy_survey_form_id input[name='option_title']");
	     ...
	     var lists6=[];
  	     ...
  	     lists6_obj.options=[];
  	     lists6_obj.id=$("#dy_add_choice_id").val();
		 ...
		 var iii=_.findWhere(save_survey_objects, {id:$("#dy_add_choice_id").val()}) ;
		 iii.question_title=lists6_obj.question_title;
		 iii.question_title=lists6_obj.question_title;
		 iii.question_type=lists6_obj.question_type;
		 iii.options=lists6_obj.options;
        
		 var pos= _.findIndex(save_survey_objects,{id:$("#dy_add_choice_id").val()});
		 //删除
		 ...
		 save_survey_objects.splice(pos,0,iii);
			 ...
			 var dy_t6_templ=_.template($("#dy_add_t6").html(),{variable: 'data'})({datas:lists6});
			     $("#"+lists6_obj.id+"abcd").empty();
	  	         $("#"+lists6_obj.id+"abcd").append(dy_t6_templ);
	  	     }else{
	  	        var dy_t8_templ=_.template($("#dy_add_t8").html(),{variable: 'data'})({datas:lists6});
	  	        $("#"+lists6_obj.id+"abcd").empty();
 	            $("#"+lists6_obj.id+"abcd").append(dy_t8_templ);
	  	  	  }
			 
			
		     $("#"+lists6_obj.id+"xyxyxy").remove();
		     $("#add_survey_lost_display").show();
		     add_edit_one_choice_fun_flag=0;
	  		 });
    ...
    $("#addDySurveyChoiceItem").click(function(){
			 var lists5_1=[1];
			 var dy_t5_templ2=_.template($("#dy_add_t5").html(),{variable: 'data'})({datas:lists5_1});
			 $("#dy_add_choice_content_div").append(dy_t5_templ2);
	 }); 
