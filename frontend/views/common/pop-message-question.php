...
<div class="header">
    ...
    <h4 class="modal-title" id="myModalLabel"><?= Yii::t('frontend', 'tab_btn_qa') ?></h4>
</div>
<div class="content">
    <table class="table table_teacher" style="margin-top:0; text-align:left;">
        <tbody>
        <tr>
            <td width="50%"><?= Yii::t('frontend', 'question') ?></td>
        ...
        $.post('<?= Url::toRoute(['common/mark-read']) ?>', {"id": id},
            function (data) {
                ...
                if (result === 'failure') {
                    ...
                    loadMessage('<?= Url::toRoute(['common/pop-message-question']) ?>', 'task1');
                }
            }, "json");
    }
