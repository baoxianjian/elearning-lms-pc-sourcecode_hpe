...
<!-- 预览问卷的弹出窗口 -->
  ...
  <div class="courseInfo">
         ...
         function get_play_survey_result(){
        	 ...
        	 $.get("<?=Url::toRoute(['investigation/get-play-investigation-submit-result',])?>"+"?course_id=<?=$courseId?>&mod_id=<?=$mod_id?>&investigation_id="+survey_submit_get_obj.id,function(data){

        		 if(data.result=='yes'){
            		 $(".blockScreen").removeClass('hide');
            		 $("#survey_play_submit_a").attr('disabled',"true");
	    		 ...
	    		 for(var i=0;i<survey_submit_get_obj.question.length;i++){
	    			 ...
	    			 if(survey_submit_to_result_obj.question_type=='2'){
						 ...
						 if($.trim($("#"+survey_submit__question.id+"wendati").val())==""){
							 app.showMsg((i+1)+'.'+survey_submit__question.question_title+"项未填");
							 submit_flag=true;
							 break ;
						 }
	    				
	    				 survey_submit_to_result_obj.option_result= $("#"+survey_submit__question.id+"wendati").val();	
	    			 ...
	    			 if(survey_submit_to_result_obj.question_type=='0'){
	    				 ...
	    				 if($.trim($("input[name='"+survey_submit__question.id+"danxuanti']:checked").val())==""){
							 app.showMsg((i+1)+'.'+survey_submit__question.question_title+"项未选择");
							 submit_flag=true;
							 break ;
						 }
	...
	<div class="row questionGroup_quest">
       <div class="col-md-12 col-sm-12">
            ...
            <%=item.question_description%>
   	 	    <textarea id="<%=item.id%>wendati"></textarea>
   	  </div>
   	</div>
      <%});%>
    </script>  
