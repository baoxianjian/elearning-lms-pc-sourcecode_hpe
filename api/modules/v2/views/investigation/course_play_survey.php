...
// deprecated by GROOT at 2016.04.21
?>
...
<!-- 预览问卷的弹出窗口 -->
  ...
  <div class="courseInfo">
                <div role="tabpanel" class="tab-pane active" id="teacher_info">
                  ...
                  <div class="infoBlock">
                    ...
                    <div class="row questionGroup_quest ">
                      <div class="hide">
                      <p>有效期：<label id="start_at_pre_id"></label>-<label id="end_at_pre_id"></label></p>
                      </div>
                      <h4 id="title_pre_id"></h4>
                     <p style="text-align:left">简介:<h4 id="description_pre_id"></h4> </p>
                    </div>
         ...
         function play_survey_res_completeFinal(){
        	 $.get("<?=Url::toRoute(['investigation/play-investigation-res-complete',])?>"+"?system_key=<?= $system_key ?>&access_token=<?= $access_token ?>&course_id=<?=$courseId?>&mod_res_id=<?=$modResId?>&course_complete_id=<?=$courseCompleteFinalId?>"+"&complete_type=1&course_reg_id="+course_play_survey_course_reg_id,function(data){
         ...
         function play_survey_res_completing(){
        	 $.get("<?=Url::toRoute(['investigation/play-investigation-res-complete',])?>"+"?system_key=<?= $system_key ?>&access_token=<?= $access_token ?>&course_id=<?=$courseId?>&mod_res_id=<?=$modResId?>&course_complete_id=<?=$courseCompleteProcessId?>"+"&complete_type=0&course_reg_id="+course_play_survey_course_reg_id,function(data){
         ...
         function get_play_survey_result_show(){
        	 ...
        	 $.get("<?=Url::toRoute(['investigation/get-sub-survey-result',])?>"+"?system_key=<?= $system_key ?>&access_token=<?= $access_token ?>&id=<?=$id?>&course_complete_id=<?=$courseCompleteProcessId?>&course_reg_id=<?=$course_reg_id?>&course_id=<?=$courseId?>&mod_id=<?=$mod_id?>&investigation_id=<?=$id?>",function(msg){
        		 $("#title_pre_id").text(msg.data.result.title);
 				$("#description_pre_id").text(msg.data.result.description);
        		 $("#pre_first_survey_row").empty();

        		 var lists_arrs=msg.data.result.question;
           	  survey_submit_get_obj=msg.data.result;
         ...
         function toYYYYMMDD(val_date){
	      ...
	      function survey_play_submit(){
	    		 ...
	    		 for(var i=0;i<survey_submit_get_obj.question.length;i++){
	    			 ...
	    			 survey_submit_to_result_obj.course_complete_id="<?=$courseCompleteProcessId?>";
...
//	    			 alert(survey_submit_to_result_obj.question_type);
	    			 if(survey_submit_to_result_obj.question_type=='2'){
						 ...
						 if($.trim($("#"+survey_submit__question.id+"wendati").val())==""){
							 app.showMsg((quest_num)+'.'+survey_submit__question.question_title+"项未填");
							 submit_flag=true;
							 break ;
						 }
						 quest_num++;
	    			 ...
	    			 if(survey_submit_to_result_obj.question_type=='0'){
	    				 ...
	    				 if($.trim($("input[name='"+survey_submit__question.id+"danxuanti']:checked").val())==""){
							 app.showMsg((quest_num)+'.'+survey_submit__question.question_title+"项未选择");
							 submit_flag=true;
							 break ;
						 }
	    				 quest_num++;
...
//							 alert(option_result);
         ...
         function survey_play_load(){
        	  console.log("----------");
        	    ...
        	    var url_result_1="<?=Url::toRoute(['investigation/course-play-survey-result',])?>"+"?system_key=<?= $system_key ?>&access_token=<?= $access_token ?>&id=<?=$id?>&course_complete_id=<?=$courseCompleteProcessId?>&course_reg_id=<?=$course_reg_id?>&course_id=<?=$courseId?>&mod_id=<?=$mod_id?>&investigation_id=";
         ...
         function init(){
        	 ...
        	 $.ajax({
            	   async: false,
    			   url: "<?=Url::toRoute(['investigation/get-survey','system_key'=>$system_key,'access_token'=>$access_token])?>",
    			   data: {id:'<?=$id?>'},
    			   success: function(msg){
       ...
       <div class="row questionGroup_quest">
                     <div class="col-md-12 col-sm-12">
	                      <div class="form-group form-group-sm">
	                        <label class="col-sm-9 control-label"><%=item.preview_num_show%>.[单选] 问题:<%=item.question_title%></label>
...
<script id="pre_t8" type="text/template">
 <%_.each(data.datas, function(item) {%>
       <div class="row questionGroup_quest">
                     <div class="col-md-12 col-sm-12">
	                      <div class="form-group form-group-sm">
	                        <label class="col-sm-9 control-label"><%=item.preview_num_show%>.[多选] 问题:<%=item.question_title%></label>
 ...
 <script id="pre_t7" type="text/template">
  <%_.each(data.datas, function(item) {%>
	<div class="row questionGroup_quest">
       <div class="col-md-12 col-sm-12">
   		   <div class="form-group form-group-sm">
   		   <label class="col-sm-9 control-label"><%=item.preview_num_show%>. 问题:<%=item.question_title%></label>
...
<%_.each(data.datas, function(item) {%>
                    ...
                    <div class=" centerBtnArea">
                      <a  class="btn btn-sm btn-success pull-right" onClick="next_page_fun('<%=item.id%>','<%=item.nid%>')">下一步</a>
                     </div>   
 <%});%>
    </script> 
...
<%_.each(data.datas, function(item) {%>
					<hr/>
                    <div class=" centerBtnArea">               
                      <a  class="btn btn-sm btn-success pull-left" onClick="pre_page_fun('<%=item.id%>','<%=item.pid%>')">上一步</a>                  
                     </div>  
 <%});%>
    </script>         
...
<%_.each(data.datas, function(item) {%>
                    ...
                    <div class=" centerBtnArea">                
                      <a  class="btn btn-sm btn-success pull-left" onClick="pre_page_fun('<%=item.id%>','<%=item.pid%>')">上一步</a>
                      <a  class="btn btn-sm btn-success pull-right" onClick="next_page_fun('<%=item.uid%>','<%=item.nid%>')">下一步</a>                     
                    </div>  
 <%});%>
    </script> 
...
<div class="row">
    <div class="col-md-12 col-sm-12 centerBtnArea">
     <a onClick="survey_play_submit()" id="survey_play_submit_a" class="btn btn-info btn-sm centerBtn" style="width:30%" data-dismiss="modal" aria-label="Close">提交</a>
   <!--   <a onClick="survey_play_load()" class="btn btn-default btn-sm centerBtn btnaddNewChoice" style="width:30%">查看结果</a>   -->
 </div>
</div>
 <%});%>
 </script>          